        ...
        <section className="py-20 px-4 text-center">
          <div className="container-custom max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-center gap-4 mb-12">
              ...
              <span className={cn("text-sm font-medium", annual ? "text-neutral-900 dark:text-white" : "text-neutral-500")}>
                Yearly <span className="text-primary-600 text-xs font-bold ml-1">(Save 20%)</span>
              </span>
            </div>
          </div>
        </section>
              ...
              {plans.map((plan, index) => (
                <Card 
                  ...
                  className={cn(
                    ...
                    plan.popular ? "border-primary-500 ring-1 ring-primary-500 shadow-xl scale-105 z-10" : "hover:shadow-lg transition-shadow"
                  )}
                >
                  {plan.popular && (
                  ...
                  <div className="mb-6">
                    ...
                    <p className="text-sm text-neutral-600 dark:text-neutral-400 h-10">{plan.description}</p>
                  </div>

                  <div className="mb-6">
                    <div className="flex items-baseline gap-1">
                      <span className="text-4xl font-bold text-neutral-900 dark:text-white">${plan.price}</span>
                      {plan.price > 0 && <span className="text-neutral-500">/{annual ? 'mo' : 'mo'}</span>}
                    </div>
                    {annual && plan.price > 0 && (
                      <p className="text-xs text-neutral-500 mt-1">Billed ${plan.price * 12} yearly</p>
                    )}
                  </div>
                  ...
                  <div className="flex-1 mb-8">
                    <ul className="space-y-3">
                      ...
                      {plan.notIncluded && plan.notIncluded.map((feature, i) => (
                        <li key={i} className="flex items-start gap-3 text-sm text-neutral-400 dark:text-neutral-600">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {faqs.map((faq, i) => (
                <div key={i} className="space-y-2">
                  <h3 className="font-bold text-lg text-neutral-900 dark:text-white">{faq.q}</h3>
